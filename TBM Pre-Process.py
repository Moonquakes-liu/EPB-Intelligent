...
# -*- coding: utf-8 -*-
# ************************************************************************
# * Software:  EPB Pre-Process  for  Python                              *
# * Version:  1.0.0                                                      *
# * Date:  2023-02-20 00:00:00                                           *
# * Last  update: 2023-02-28 00:00:00                                    *
# * License:  LGPL v1.0                                                  *
# * Maintain  address:  https://pan.baidu.com/s/1GOL5pxNTS44RK7QVKZavCQ  *
# * Maintain  code:  SEPB                                                *
# ************************************************************************
...
class EPB_REPORT(object):
    ...
    ****可选参数：添加封面（cover），可选选项为（True/False），默认为开启状态;
    ...
    def add_cover_info(self, Obj) -> None:
        ...
        sheet = Table(data=[['%s' % current_year], ['EPB数据预处理报告书'], ['EPB DATA PREPROCESSING REPORT'], ['**机构信息**']],
    ...
    def MergePDF(self, pdf_path: list) -> None:  # 规定返回值无类型限定/无返回值
        ...
        merger.write(os.path.join(self.out, 'EPB-Data.pdf'))  # 将合并后的pdf写入到新文件
        merger.close()  # 关闭文件
...
class MESSAGE(object):
    ...
    def __init__(self):
        self.root = Tk()
        self.Dir = {'Cycle': ['', '[ EPB_CYCLE ]-AllDataSet'],
                    'Extract': ['[ EPB_CYCLE ]-AllDataSet', '[ EPB_EXTRACT ]-KeyDataSet'],
                    'Class': ['[ EPB_EXTRACT ]-KeyDataSet', '[ EPB_CLASS ]-Class-Data'],
                    'Clean': ['[ EPB_CLASS ]-Class-Data', '[ EPB_CLEAN ]-Clean-Data'],
                    'Merge': ['[ EPB_CLEAN ]-Clean-Data', '[ EPB_MERGE ]-ML-Data'],
                    'Filter': ['[ EPB_MERGE ]-ML-Data', '[ EPB_FILTER ]-ML2-Data'],
                    'Split': ['[ EPB_MERGE ]-ML-Data', '[ EPB_SPLIT ]-A-ML2-Data'],
                    'Plot': ['[ EPB_MERGE ]-ML-Data', '[ EPB_PLOT ]-Pic'],
                    'Report': ['[ EPB_MERGE ]-ML-Data', '[ EPB_REPORT ]-Pdf-Data']}  # 输入输出文件夹路径
    ...
    def Output_Event(self, var, fuc=None, Type='Dir', file_type=None):
        ...
        if Type == 'File':
            name = 'backup EPB PreProcess %s.tar.gz' % datetime.date.today()
    ...
    def Bottom_Button(self, Framework):
        url = 'https://pan.baidu.com/s/1GOL5pxNTS44RK7QVKZavCQ?pwd=SEPB'
    ...
    def Default_Setting():
        ...
        Report = {'USING-MODEL': False, 'DEBUG': False, 'INPUT-PATH': '--请选择--',
                  ...
                  'WATERMARK': False, 'OUTER': False, 'WATERMARK-INFO': 'EPB掘进参数'}
...
def Check_Update():
    ...
    URL = "https://github.com/Moonquakes-liu/EPB-Intelligent/archive/refs/heads/main.zip"
    try:
...
if __name__ == "__main__":
    ...
    if CONF[1]['USING-MODEL']:
        EPB_CYCLE(input_path=CONF[1]['INPUT-PATH'], out_path=CONF[1]['OUTPUT-PATH'], parameter=CONF[1]['PARAMETER'],
    ...
    if CONF[2]['USING-MODEL']:
        EPB_EXTRACT(input_path=CONF[2]['INPUT-PATH'], out_path=CONF[2]['OUTPUT-PATH'], key_parm=CONF[2]['PARAMETER'],
                    index_path=CONF[0]['INDEX-PATH'], debug=CONF[2]['DEBUG'])
    if CONF[3]['USING-MODEL']:
        EPB_CLASS(input_path=CONF[3]['INPUT-PATH'], project_type=CONF[3]['PROJECT'], L_min=CONF[3]['LENGTH-MIN2'],
    ...
    if CONF[4]['USING-MODEL']:
        EPB_CLEAN(input_path=CONF[4]['INPUT-PATH'], out_path=CONF[4]['OUTPUT-PATH'], debug=CONF[4]['DEBUG'],
                  parameter=CONF[4]['PARAMETER'], V_max=CONF[4]['VELOCITY-MAX2'], index_path=CONF[0]['INDEX-PATH'])
    if CONF[5]['USING-MODEL']:
        EPB_MERGE(input_path=CONF[5]['INPUT-PATH'], out_path=CONF[5]['OUTPUT-PATH'], parameter=CONF[5]['PARAMETER'],
                  debug=CONF[5]['DEBUG'], index_path=CONF[0]['INDEX-PATH'])
    if CONF[6]['USING-MODEL']:
        EPB_FILTER(input_path=CONF[6]['INPUT-PATH'], out_path=CONF[6]['OUTPUT-PATH'], parameter=CONF[6]['PARAMETER'],
                   debug=CONF[6]['DEBUG'], index_path=CONF[0]['INDEX-PATH'])
    if CONF[7]['USING-MODEL']:
        EPB_SPLIT(input_path=CONF[7]['INPUT-PATH'], out_path=CONF[7]['OUTPUT-PATH'], debug=CONF[7]['DEBUG'],
                  parameter=CONF[7]['PARAMETER'], partition=CONF[7]['PARTITION-METHOD'], min_time=CONF[7]['TIME-MIN'],
                  index_path=CONF[0]['INDEX-PATH'])
    if CONF[8]['USING-MODEL']:
        EPB_PLOT(input_path=CONF[8]['INPUT-PATH'], out_path=CONF[8]['OUTPUT-PATH'], parameter=CONF[8]['PARAMETER'],
    ...
    if CONF[9]['USING-MODEL']:
        EPB_REPORT(input_path=CONF[9]['INPUT-PATH'], out_path=CONF[9]['OUTPUT-PATH'], parameter=CONF[9]['PARAMETER'],
